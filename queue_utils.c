...
/*   Updated: 2026/09/07 09:24:56 by vivozzo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heapify_down(t_dongle *dongle, int idx, t_hub *hub)
{
	...
	while (1)
	{
		...
		left = 2 * idx + 1;
		right = 2 * idx + 2;
		if (left < dongle->coder_queue_size && has_higher_priority(hub,
				dongle->coder_queue[left], dongle->coder_queue[smallest]))
			smallest = left;
		if (right < dongle->coder_queue_size && has_higher_priority(hub,
				dongle->coder_queue[right], dongle->coder_queue[smallest]))
			smallest = right;
		if (smallest == idx)
			break ;
...
void	queue_pop(t_dongle *dongle, t_hub *hub)
{
	if (dongle->coder_queue_size == 0)
		return ;
	dongle->coder_queue[0] = dongle->coder_queue[dongle->coder_queue_size - 1];
	dongle->coder_queue_size--;
