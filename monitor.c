...
/*   Updated: 2026/09/04 11:32:40 by vivozzo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int monitor_check_burnout(t_hub *hub, int index)
{
    ...
    if (hub->args->number_of_compiles_required ==
        hub->coders[index].total_compiles)
        return (pthread_mutex_unlock(&hub->coders[index].coder_mutex), 0);
...
{
    if (pthread_create(&hub->monitor_thread_id, NULL,
                       monitor_thread, hub) != 0)
        return (-1);
    return (0);
}
